...
def _merge_to_parent_level(docs: List[dict], threshold: int = 2) -> Tuple[List[dict], int]:
    ...
    #一次批量查询 PostgreSQL + Redis（get_documents_by_ids 先查 Redis 缓存，miss 再查 PG）。构建 {chunk_id: doc} 映射表，后面 O(1) 查找。
    ...
    #为什么会有重复？ 上一步里，假如c0、c1、c2 三个子块都被替换为同一个父块，merged_docs 变成 [父块, 父块, 父块, c3, c4]。去重后只保留第一次出现的 [父块, c3, c4]
    deduped: List[dict] = []
    seen = set()
    for item in merged_docs:
        #优先: chunk_id (最可靠的唯一标识)
        #兜底: (filename, page_number, text) 三元组 (万一 chunk_id 缺失)
...
def retrieve_documents(query: str, top_k: int = 5, *, collection_name: str) -> Dict[str, Any]:
    _milvus_manager.init_collection(collection_name)
