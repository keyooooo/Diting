...
from app.rag.retrieval import retrieve_documents, step_back_expand, generate_hypothetical_document
from app.agent.tools import emit_rag_step
...
_grader_model = None #评估模型
_router_model = None #路由模型
...
def _get_grader_model():
    ...
    if _grader_model is None:
        _grader_model = init_chat_model(
            ...
            api_key=API_KEY,
            base_url=BASE_URL,
            temperature=0, #评判任务需要确定性，不需要随机性
            stream_usage=True, #传这个参数。用于计费和监控。
        )
...
def _format_docs(docs: List[dict]) -> str:
    '''
    纯文本格式化逻辑，不在主流程里写拼接。函数式思维——输入 docs 列表，输出 LLM 能消费的格式化文本。
    '''
    if not docs:
        return ""
    chunks = []
...
def retrieve_initial(state: RAGState) -> RAGState:
    ...
    retrieved = retrieve_documents(query, top_k=5, collection_name=state["collection_name"])
    ...
    if not results:
        return {
            ...
            "docs": [],
            "context": "",
            "rag_trace": {"query": query, "retrieval_stage": "initial", "retrieved_chunks": []},
        }
...
def grade_documents_node(state: RAGState) -> RAGState:
    ...
    if not grader:
        # 为什么没 grader 就走 rewrite？ 宁可多余地重写一次查询，也不要拿着不相关的检索结果去生成答案（用户收到的答复会是无意义的幻觉）。这是一种"安全偏向"——在信息不足时选择保守行为。
        grade_update = {
            "grade_score": "unknown",
...
def retrieve_expanded(state: RAGState) -> RAGState:
    ...
    # 为什么要提前声明这么多变量？因为这个节点里最多会调两次 retrieve_documents（HyDE + Step-back），每次调用都返回自己的 meta。需要合并它们——对同一个字段，两路的合并方式不同
    results: List[dict] = []
    ...
    if strategy in ("hyde", "complex"):
        hypothetical_doc = state.get("hypothetical_doc") or generate_hypothetical_document(state["question"]) #万一 rewrite_question_node 异常没产出假设文档，这里再紧急生成一次，不阻塞管道。
        retrieved_hyde = retrieve_documents(hypothetical_doc, top_k=5, collection_name=state["collection_name"])
    ...
    if strategy in ("step_back", "complex"):
        ...
        retrieved_stepback = retrieve_documents(expanded_query, top_k=5, collection_name=state["collection_name"])
...
def run_rag_graph(question: str, *, collection_name: str) -> dict:
