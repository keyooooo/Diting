...
def get_current_weather(location: str, extensions: Optional[str] = "base") -> str:
    """获取指定城市的实时或预报天气。location 必须是纯城市名（如"佛山""北京"）或行政区划代码（如440600），不要带省前缀。"""
    if not location:
...
def create_search_knowledge_tool(collection_name: str):
    """按 collection 创建知识库检索工具（闭包注入 collection_name）。"""
    ...
    def search_knowledge_base(query: str) -> str:
        ...
        from app.rag.pipeline import run_rag_graph

        rag_result = run_rag_graph(query, collection_name=collection_name)
