...
createApp({
  ...
  beforeUnmount() {
    ...
    Object.values(this.deleteRemoveTimers).forEach((timer) =>
      clearTimeout(timer),
    );
  },
  methods: {
    ...
    async handleAuthSubmit() {
      ...
      try {
        const endpoint =
          this.authMode === "login" ? "/auth/login" : "/auth/register";
        const payload = {
          username,
          password,
        };
    ...
    async loadDocuments() {
      ...
      try {
        const url = this.uploadKbId ? `/documents?kb_id=${this.uploadKbId}` : "/documents";
    ...
    uploadFileWithProgress(file) {
      return new Promise((resolve, reject) => {
        ...
        if (this.uploadKbId) {
          formData.append("kb_id", this.uploadKbId);
        }
    ...
    startUploadJobPolling(jobId) {
      ...
      const poll = async () => {
        try {
          const response = await this.authFetch(
            `/documents/upload/jobs/${encodeURIComponent(jobId)}`,
          );
          if (!response.ok) {
    ...
    syncDeleteJob(filename, job) {
      ...
      this.setDeleteJob(filename, {
        ...
        message: job.message || "",
        collapsed:
          job.status === "completed" ? true : Boolean(current.collapsed),
    ...
    startDeleteJobPolling(filename, jobId) {
      ...
      const poll = async () => {
        try {
          const response = await this.authFetch(
            `/documents/delete/jobs/${encodeURIComponent(jobId)}`,
          );
          if (!response.ok) {
            const error = await response.json().catch(() => ({}));
    ...
    async deleteDocument(filename) {
      ...
      if (
        !confirm(
          `确定要删除文档 "${filename}" 吗？这将同时删除 Milvus 中的所有相关向量。`,
        )
      ) {
        return;
      }
      ...
      try {
        let deleteUrl = `/documents/delete/async/${encodeURIComponent(filename)}`;
        if (this.uploadKbId) deleteUrl += `?kb_id=${this.uploadKbId}`;
        const response = await this.authFetch(deleteUrl, { method: "DELETE" });
    ...
    async loadKnowledgeBases() {
      try {
        const response = await this.authFetch("/knowledge-bases");
        if (response.ok) {
          const data = await response.json();
    ...
    async createKnowledgeBase() {
      ...
      try {
        const response = await this.authFetch("/knowledge-bases", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ name, description: this.newKbDesc.trim() }),
        });
        if (response.ok) {
          this.newKbName = "";
          this.newKbDesc = "";
    ...
    async deleteKnowledgeBase(kbId) {
      if (!confirm("确定要删除该知识库？其中的文档数据不会被删除，但需要重新关联。")) return;
      try {
        const response = await this.authFetch(`/knowledge-bases/${kbId}`, { method: "DELETE" });
        if (response.ok) {
          if (this.selectedKbId === kbId) this.selectedKbId = null;
          if (this.uploadKbId === kbId) this.uploadKbId = null;
          await this.loadKnowledgeBases();
        } else {
          const data = await response.json().catch(() => ({}));
